...
function when(value, fulfilled, rejected) {
    ...
    function _fulfilled(value) {
        try {
            ...
            exception._stack = exception.stack.split('\n');
            console.error("when.fulfillled ERROR "+exception, exception);
...
function end(promise) {
    when(promise, undefined, function (error) {
        ...
        nextTick(function () {
            var chainedError = new Error(error.toString());
            chainedError.originalStack = error.stack;
            console.error(error+"", error.stack.split('/n'));
            throw chainedError;
        });
    });
}
